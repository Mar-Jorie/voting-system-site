...
const CandidatesPage = () => {
  ...
  // Auto-download functionality removed - PDF downloads only happen when user clicks export button
  ...
  const handleConfirmSave = async () => {
    setSaveLoading(true);
    let candidateData = null; // Declare outside try block to fix scope issue
    ...
    try {
      ...
      if (!pendingFormData.name || !pendingFormData.category) {
      ...
      if (error.message && (error.message.includes('413') || error.message.includes('Content Too Large') || error.message.includes('request entity too large'))) {
        toast.error('Files too large. Please reduce image sizes or upload fewer images. Maximum 1MB per file.');
        ...
        toast.error(`Failed to save candidate: ${error.message || 'Unknown error'}`);
      }
      setShowConfirmModal(false);
      setPendingFormData(null);
    } finally {
  ...
  const exportAsPDF = async () => {
    try {
      ...
      const tableData = candidatesToExport.map(candidate => ({
        name: candidate.name,
        category: candidate.category,
      ...
      const filename = `candidates-export-${timestamp}.pdf`;
      ...
      // Generate PDF using the utility
            await generateTablePDF(
              ...
              filename,
              {
                ...
                subtitle: `Total Candidates: ${candidatesToExport.length}`,
                showRowNumbers: true
              }
            );
            
            setShowExportModal(false);
            toast.success(`${candidatesToExport.length} candidate(s) exported as PDF successfully`);
      
      // Log export operation
      await auditLogger.log({
        action: 'export_data',
        entity_type: 'candidates',
        entity_name: 'Candidates Export',
        details: {
          format: 'PDF',
          record_count: candidatesToExport.length,
          filename: filename
        },
      ...
      <FormModal
        ...
        fields={[
          ...
          },
          {
            ...
            multiple: true,
            maxFiles: 3,
            maxSize: 500 * 1024, // 500KB - further reduced to avoid server limits
            accept: 'image/*',
