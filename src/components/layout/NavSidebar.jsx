...
const NavSidebar = () => {
  ...
  const handleLogoutClick = () => {
    // Close sidenav on mobile/tablet when sign-out modal appears
    if (isMobile || isTablet) {
      setShow(false);
    }
  ...
  const navigationItems = [
    {
      ...
      href: `/${user?.username || ''}`,
      icon: HomeIcon,
      current: location.pathname === `/${user?.username || ''}` || location.pathname === '/'
    },
    {
      name: 'Candidates',
      href: `/${user?.username || ''}/candidates`,
      icon: GlobeAltIcon,
      current: location.pathname === `/${user?.username || ''}/candidates`
    },
    {
      name: 'FAQ',
      href: `/${user?.username || ''}/faq`,
      icon: QuestionMarkCircleIcon,
      current: location.pathname === `/${user?.username || ''}/faq`
    },
        {
          name: 'Votes List',
          href: `/${user?.username || ''}/votes`,
          icon: ListBulletIcon,
          current: location.pathname === `/${user?.username || ''}/votes`
        },
    {
      name: 'Users',
      href: `/${user?.username || ''}/users`,
      icon: UserIcon,
      current: location.pathname === `/${user?.username || ''}/users`
    }
  ];
          ...
          <div className="hidden lg:block">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 flex items-center justify-center">
                <img src="/voting-system-site/logo.png" alt="Logo" className="w-6 h-6 object-contain" />
              </div>
              <div>
                <h1 className="text-base font-medium text-gray-900">
                  Voting System
